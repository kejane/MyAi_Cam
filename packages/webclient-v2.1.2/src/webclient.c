 ...
 *         >0: response http status code
 */
int webclient_post(struct webclient_session *session, const char *URI, const char *post_data, int post_len)
{
    int rc = WEBCLIENT_OK;
    int resp_status = 0;
    ...
    if (post_data)
    {
        webclient_write(session, (unsigned char *) post_data, post_len);
...
int webclient_request(const char *URI, const char *header, const char *post_data, unsigned char **response)
{
    ...
    else
    {
        ...
        if (webclient_post(session, URI, post_data, srelen(post_data)) != 200)
        {
            rc = -WEBCLIENT_ERROR;
            goto __exit;
        }
