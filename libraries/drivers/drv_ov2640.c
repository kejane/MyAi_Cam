...
#define EVENT_FLAG1  (1<<1)//ON

volatile rt_uint32_t rgb565_data_len = 0;
volatile rt_uint8_t  rgb565_data_ok  = 0;
struct rt_i2c_bus_device *i2c_bus  = RT_NULL;
extern struct rt_event my_event;

#define RGB565_BUF_SIZE   150 * 1024
#define RGB565_LINE_SIZE  640
#define Grayscale_BUF_SIZE 320*240*1


static rt_uint32_t *rgb565_data_buf = RT_NULL;
static rt_uint8_t *grayscale_data_buf=RT_NULL;
/*static rt_uint32_t JPEG_LINE0_BUF[JPEG_LINE_SIZE * 2];
static rt_uint32_t JPEG_LINE1_BUF[JPEG_LINE_SIZE * 2];*/
static rt_uint32_t RGB565_LINE0_BUF[RGB565_LINE_SIZE*2];
static rt_uint32_t RGB565_LINE1_BUF[RGB565_LINE_SIZE*2];
extern int ifperson_app(rt_uint8_t array_in_320X240[][320]);
...
static rt_err_t write_reg(struct rt_i2c_bus_device *bus, rt_uint8_t reg, rt_uint8_t data)
{
   ...
   /* if (rt_i2c_transfer(bus, &msgs, 1) == 1)
    ...
    value=rt_i2c_transfer(bus, &msgs, 1);
        if(value==1)    return RT_EOK;
        else LOG_D("%d",value);
        return RT_ERROR;
    return RT_ERROR;
}
...
void ov2640_set_special_effects(rt_uint8_t eft)
{
    ...
    write_reg(i2c_bus, 0x7d, reg7d1val);//灰度可修改80.80
...
void camera_dma_data_process(void)//DMA中断回调函数
{
   rt_uint16_t i;
   rt_uint32_t *pbuf;
   pbuf = rgb565_data_buf + rgb565_data_len;//32K的图像地址（初始）+已读的数据量=开始写数据的地址

   if (DMA2_Stream1->CR & (1<<19))//CT = 0：DMA正在访问存储区0，CPU可以访问存储区1;CT = 1：DMA正在访问存储区1，CPU可以访问存储区0
   {
       for (i = 0; i < RGB565_LINE_SIZE; i++)
       {
           pbuf[i] = RGB565_LINE0_BUF[i];
       }
       rgb565_data_len += RGB565_LINE_SIZE;
   }
   else
   {
       for (i = 0; i < RGB565_LINE_SIZE; i++)
       {
           pbuf[i] = RGB565_LINE1_BUF[i];
       }
       rgb565_data_len += RGB565_LINE_SIZE;
   }
   SCB_CleanInvalidateDCache();//数据传输完毕，清理Dcache
}

/* After a frame of picture data has been collected. */
void camera_frame_data_process(void)//DCMI 回调函数
{
   rt_uint16_t i, rlen;
   rt_uint32_t *pbuf = RT_NULL;

   if (rgb565_data_ok == 0)//还未读取数据
   {
       __HAL_DMA_DISABLE(&hdma_dcmi);//禁用DMA
       while(DMA2_Stream1->CR & 0x01);//等待DMA失能

       rlen = RGB565_LINE_SIZE - __HAL_DMA_GET_COUNTER(&hdma_dcmi);//总数据-未传输完的数据=已读数据
       pbuf = rgb565_data_buf + rgb565_data_len;//开始写数据的地址

       if (DMA2_Stream1->CR & (1<<19))//把正在传输的数据通道的数据传输完毕
       {
           for (i = 0; i < rlen; i++)
           {
               pbuf[i] = RGB565_LINE1_BUF[i];
       ...
       rgb565_data_len += rlen;
       rgb565_data_ok   = 1;//传输完毕 ok=1；
   }
   if (rgb565_data_ok==2)
   {
       DMA2_Stream1->NDTR = RGB565_LINE_SIZE;
       DMA2_Stream1->CR  |= 1<<0;
       rgb565_data_ok  = 0;
       rgb565_data_len = 0;
   }
...
int rt_ov2640_init(void)
{
    ...
    result = write_reg(i2c_bus, OV2640_DSP_RA_DLMT, 0x01);//操作sensor寄存器
    if (result != RT_EOK )
    {
    ...
    rt_thread_delay(10);
    result = write_reg(i2c_bus, OV2640_SENSOR_COM7, 0x80);//软复位ov2640
    if (result != RT_EOK)
    {
    ...
    result = ov2640_read_id(i2c_bus);//读ID
    if (result != RT_EOK )
    {
    ...
    {
        write_reg(i2c_bus, ov2640_sxga_init_reg_tbl[i][0], ov2640_sxga_init_reg_tbl[i][1]);
    }
    ...
    //ov2640_jpeg_mode();
    ov2640_set_image_window_size(0, 0, 1600, 1200);
   ...
   grayscale_data_buf=rt_malloc(Grayscale_BUF_SIZE);
   if (RT_NULL == rgb565_data_buf)
   {
    ...
    /* start dcmi capture */
    rt_hw_dcmi_dma_config((rt_uint32_t)RGB565_LINE0_BUF, (rt_uint32_t)RGB565_LINE1_BUF, RGB565_LINE_SIZE);//配置传输地址和传输大小
...
void camera_on(void)
{
   rt_uint8_t R,G,B,Gray,value,start_flag;
   rt_uint32_t i,j, rgb565_len;
   rt_uint16_t *p = RT_NULL;
   rt_uint8_t (*dest)[320]=(rt_uint8_t(*)[320])grayscale_data_buf;
   while(1)
      {
       value=Remote_Scan();
      if(value==56)//value有变，
      ...
      {
           DCMI_Start();
           rt_kprintf("DCMI_Start ok\r\n");
          while (rgb565_data_ok != 1);//等待数据被读取

          DCMI_Stop();//传输完成 ，DCMI停止

          p = (rt_uint16_t *)rgb565_data_buf;
          rgb565_len  = 320*240;
          ...
          if (rgb565_len)
          {
              for(i=0;i<240;i++)
              {
                  for(j=0;j<320;j++)
                  {
                      ...
                      G = G<<2;//*4
                      R = R<<3;//*8
                      Gray=  (rt_uint8_t)( (R*38 + G*75 + B*15) >> 7);
                      dest[i][j]=Gray;
                      p++;
                  }
              }
...
MSH_CMD_EXPORT(camera_on, start ifperson app);
